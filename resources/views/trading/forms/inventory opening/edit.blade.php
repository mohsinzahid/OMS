            ...
            <div class="col-sm-6">
                ...
                <div class="panel panel-filled">
                    <div class="panel-body">
                        <form method="post" action="{{ url('/stock/Updatestock') }}" autocomplete="off">
                            {{ csrf_field() }}

                            <input type="text" class="hidden" value="{{$stock->id}}" name="stockid">
                            <input type="text" class="hidden" value="{{$size->id}}" name="sizeid">


                            <div class="form-group">
                                <label for="InputName">Size</label>
                                <input type="text" class="form-control" id="InputName" value="{{$size->size}}"
                                       placeholder="eg : 123x123" name="sizename" required>
                            </div>

                            <div class="form-group">
                                <label for="InputQuantity">Quantity</label>
                                <input type="text" class="form-control" id="InputQuantity" pattern="\d*" value="{{$stock->quantity}}" placeholder="0" name="quantity" required>
                            </div>

                            <div class="form-group">
                                <label for="Inputprice">Sale Price </label>
                                <input type="text" class="form-control" id="Inputsale" pattern="[+-]?([0-9]+([.][0-9]*)?|[.][0-9]+)" value="{{$stock->saleprice}}" placeholder="0.00" name="saleprice" required>
                            </div>
